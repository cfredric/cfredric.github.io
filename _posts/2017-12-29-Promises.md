...
have multiple results, functions that read from an environment, functions that
also produce a log of what they did, ...), there are all kinds of monads, and
they are extremely useful -- in practice, not just in academia. Hopefully this
post has demystified them, and illustrated that they're a useful way of
composing functions into pipelines, without drowning in boilerplate.
