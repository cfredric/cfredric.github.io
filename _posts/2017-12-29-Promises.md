...
another function to produce another of the same container data type, holding
something else. `return` takes a normal value and puts it in a "minimal"
version of that container type. (In the case of `Maybe`, there's only one way
to put a value in the container -- by using `Just`.) The full, real code
