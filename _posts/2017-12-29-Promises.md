...
**The Good.** `Promise`s keep track of errors as control flow moves through the
...
`Promise`s don't require any baked-in language support (they're implemented in
...
One way to achieve this is to teach the language to understand `Promise`s. The
language will provide a way to "pause" a function until a given `Promise`
resolves, and it'll provide a way to handle `Promise`s that reject along the
way.  (Of course, the function wouldn't *really* pause, since that would defeat
the purpose of the asynchronous call.) These are the ideas behind the
